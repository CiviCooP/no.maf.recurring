...
<table class="form-layout-compressed">
    ...
    <!-- customized andyw@circle, 09/09/2013 - add form for ContributionRecur / KID Number -->
    <tr>
        <td class="label">{$form.create_contribution_recur.label}</td>
        <td style>{$form.create_contribution_recur.html}</td>
    </tr>
</table>

{literal}
<script type="text/javascript">
  cj('#create_contribution_recur').click(function() {
    cj('.contributionRecurForm').animate({ height: 'toggle', opacity: 'toggle'});
  });
</script>
{/literal}

<div class="contributionRecurForm">
    <table class="form-layout-compressed">
        <tr><td class="label">{$form.amount.label}</td><td>{$form.amount.html}</td><tr>
        <tr><td>&nbsp;</td><td><p><strong>{$form.is_recur.html} {ts}{$form.frequency_interval.label}{/ts} &nbsp;{$form.frequency_interval.html} &nbsp; {$form.frequency_unit.html}<!--&nbsp; {ts}for{/ts} &nbsp; {$form.installments.html} &nbsp;{$form.installments.label}--></strong></td></tr>
        ...
        <tr>
            ...
            <td>{$form.notification_for_bank.html}</td>
        </tr>
